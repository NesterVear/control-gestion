...
def actualizar_captura(folio_acaac):
    ...
    try:
        ...
        captura.fecha_recepcion = datetime.strptime(data['fecha_recepcion'], '%d-%m-%Y').date() if data.get('fecha_recepcion') else captura.fecha_recepcion
...
def eliminar_captura(folio_acaac):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'message': 'Captura Marcada como Eliminada'})
    except Exception as e:
        db.session.rollback()
        return jsonify({'Error': str(e)}), 400
...
def test_alertas():
    ...
    return jsonify({'message': 'Alertas probadas'})
